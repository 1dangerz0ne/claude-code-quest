...
create table if not exists achievements (
  id text primary key, -- e.g., 'first_blood', 'speed_demon'
  name text not null,
  description text not null,
  icon text not null, -- emoji or icon name
  category text not null, -- 'milestone', 'skill', 'social', 'special'
...
create table if not exists challenges (
  ...
  question_seed int not null, -- Seed for deterministic questions
  created_at timestamptz default now(),
...
create table if not exists challenge_attempts (
  ...
  guest_name text, -- Allow guests to play
  score int not null,
  correct int not null,
  total int not null,
  time_seconds int,
...
create table if not exists activity_log (
  id uuid default gen_random_uuid() primary key,
  activity_type text not null, -- 'game_complete', 'achievement_unlock', 'level_up'
  user_id uuid references profiles(id) on delete cascade,
  metadata jsonb,
  created_at timestamptz default now()
);
