...
-- CLEAN INSTALL - Drops existing policies first
...
-- Drop existing policies first (profiles)
drop policy if exists "Users can view own profile" on profiles;
drop policy if exists "Users can insert own profile" on profiles;
drop policy if exists "Users can update own profile" on profiles;

-- Drop existing policies (category_progress)
drop policy if exists "Users can view own category progress" on category_progress;
drop policy if exists "Users can insert own category progress" on category_progress;
drop policy if exists "Users can update own category progress" on category_progress;

-- Drop existing policies (daily_results)
drop policy if exists "Users can view own daily results" on daily_results;
drop policy if exists "Users can insert own daily results" on daily_results;
drop policy if exists "Anyone can view daily leaderboard" on daily_results;

-- Drop existing policies (game_sessions)
...
drop trigger if exists category_progress_updated_at on category_progress;
